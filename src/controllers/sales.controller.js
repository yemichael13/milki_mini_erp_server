...
const accountantApprove = async (req, res, next) => {
  try {
    const tx = await salesService.accountantApprove(req.user, Number(req.params.id));
    res.json(tx);
  } catch (err) {
    next(err);
  }
...
const managerApprove = async (req, res, next) => {
  try {
    const tx = await salesService.managerApprove(req.user, Number(req.params.id));
    res.json(tx);
  } catch (err) {
    next(err);
  }
...
const reject = async (req, res, next) => {
  try {
    const tx = await salesService.reject(req.user, Number(req.params.id), req.body.rejection_reason);
    res.json(tx);
  } catch (err) {
    next(err);
  }
