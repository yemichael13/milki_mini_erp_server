...
const resubmit = async (req, res, next) => {
  try {
    ...
    const tx = await procurementService.reject(req.user, Number(req.params.id), reason, receiptUrl);
    res.json(tx);
  } catch (err) {
    next(err);
  }
...
const accountantApprove = async (req, res, next) => {
  try {
    ...
    const receiptUrl = req.file?.path || null;
    const tx = await procurementService.accountantApprove(req.user, Number(req.params.id), description, receiptUrl);
    res.json(tx);
  } catch (err) {
    next(err);
  }
...
const managerApprove = async (req, res, next) => {
  try {
    const description = req.body.description;
    const receiptUrl = req.file?.path || null;
    const tx = await procurementService.managerApprove(req.user, Number(req.params.id), description, receiptUrl);
    res.json(tx);
  } catch (err) {
    next(err);
  }
