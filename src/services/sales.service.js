const salesTxRepo = require("../repositories/salesTransaction.repository");
const customerRepo = require("../repositories/customer.repository");

const listMine = async (userId, status) => {
  return salesTxRepo.listMine(userId, status || null);
};

const listAll = async (status) => {
  return salesTxRepo.listAll(status || null);
};

const getById = async (id) => {
  const tx = await salesTxRepo.findById(id);
  if (!tx) {
    const err = new Error("Sales transaction not found");
    err.statusCode = 404;
    throw err;
  }
  return tx;
...
const create = async (user, data, receiptUrl) => {
  ...
  const id = await salesTxRepo.create({
    ...data,
    receipt_url: receiptUrl || null,
    created_by: user.id,
  });
  return salesTxRepo.findById(id);
};

const uploadReceipt = async (user, id, receiptUrl) => {
  const tx = await getById(id);
  if (user.role !== "sales_officer" || tx.created_by !== user.id) {
    const err = new Error("Forbidden");
    err.statusCode = 403;
    throw err;
  }
  ...
  await salesTxRepo.updateReceipt(id, receiptUrl);
  return salesTxRepo.findById(id);
};
...
const resubmit = async (user, id, data) => {
  ...
  await salesTxRepo.resubmit(id, data);
  return salesTxRepo.findById(id);
};
...
const accountantApprove = async (user, id, description, receiptUrl) => {
  ...
  await salesTxRepo.setAccountantApproved(id, user.id, description, receiptUrl);
  return salesTxRepo.findById(id);
};
...
const managerApprove = async (user, id, description, receiptUrl) => {
  ...
  await salesTxRepo.setManagerApproved(id, user.id, description, receiptUrl);
  return salesTxRepo.findById(id);
};
...
const reject = async (user, id, reason, receiptUrl) => {
  ...
  await salesTxRepo.setRejected(id, user.id, reason, receiptUrl);
  return salesTxRepo.findById(id);
};
